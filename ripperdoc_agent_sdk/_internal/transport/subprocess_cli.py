...
from ...types import RipperdocAgentOptions
from . import Transport

logger = logging.getLogger(__name__)

_DEFAULT_MAX_BUFFER_SIZE = 1024 * 1024  # 1MB buffer limit
MINIMUM_RIPPERDOC_CODE_VERSION = "0.4.0"


class SubprocessCLITransport(Transport):
    """Subprocess transport using Ripperdoc Code CLI."""
    ...
    def _find_cli(self) -> str:
        """Find Ripperdoc Code CLI binary."""
        ...
        locations = [
            Path.home() / ".npm-global/bin/ripperdoc",
            Path("/usr/local/bin/ripperdoc"),
            Path.home() / ".local/bin/ripperdoc",
            Path.home() / "node_modules/.bin/ripperdoc",
            Path.home() / ".yarn/bin/ripperdoc",
            Path.home() / ".ripperdoc/local/ripperdoc",
        ]
        ...
        raise CLINotFoundError(
            ...
            "  npm install -g @anthropic-ai/ripperdoc-code\n"
    ...
    def _find_bundled_cli(self) -> str | None:
        ...
        if bundled_path.exists() and bundled_path.is_file():
            logger.info(f"Using bundled Ripperdoc Code CLI: {bundled_path}")
    ...
    def _build_command(self) -> list[str]:
        ...
        if self._options.tools is not None:
            ...
            if isinstance(tools, list):
                ...
                # Preset object - 'ripperdoc_code' preset maps to 'default'
    ...
    async def connect(self) -> None:
        ...
        if not os.environ.get("RIPPERDOC_AGENT_SDK_SKIP_VERSION_CHECK"):
            await self._check_ripperdoc_version()
        ...
        try:
            ...
            process_env = {
                ...
                **self._options.env,  # User-provided env vars
                "RIPPERDOC_CODE_ENTRYPOINT": "sdk-py",
                "RIPPERDOC_AGENT_SDK_VERSION": __version__,
            }

            # Enable file checkpointing if requested
            if self._options.enable_file_checkpointing:
                process_env["RIPPERDOC_CODE_ENABLE_SDK_FILE_CHECKPOINTING"] = "true"
            ...
            if self._cwd and not Path(self._cwd).exists():
                ...
                raise error from e
            error = CLINotFoundError(f"Ripperdoc Code not found at: {self._cli_path}")
            self._exit_error = error
            raise error from e
        except Exception as e:
            error = CLIConnectionError(f"Failed to start Ripperdoc Code: {e}")
            self._exit_error = error
            raise error from e
    ...
    async def _check_ripperdoc_version(self) -> None:
        """Check Ripperdoc Code version and warn if below minimum."""
            ...
            with anyio.fail_after(2):  # 2 second timeout
                ...
                if version_process.stdout:
                    ...
                    if match:
                        ...
                        min_parts = [
                            int(x) for x in MINIMUM_RIPPERDOC_CODE_VERSION.split(".")
                        ]

                        if version_parts < min_parts:
                            warning = (
                                f"Warning: Ripperdoc Code version {version} is unsupported in the Agent SDK. "
                                f"Minimum required version is {MINIMUM_RIPPERDOC_CODE_VERSION}. "
