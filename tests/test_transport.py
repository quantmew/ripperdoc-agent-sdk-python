"""Tests for Ripperdoc SDK transport layer."""
...
from ripperdoc_agent_sdk._internal.transport.subprocess_cli import SubprocessCLITransport
from ripperdoc_agent_sdk.types import RipperdocAgentOptions

DEFAULT_CLI_PATH = "/usr/bin/ripperdoc"


def make_options(**kwargs: object) -> RipperdocAgentOptions:
    """Construct options using the standard CLI path unless overridden."""

    cli_path = kwargs.pop("cli_path", DEFAULT_CLI_PATH)
    return RipperdocAgentOptions(cli_path=cli_path, **kwargs)
...
class TestSubprocessCLITransport:
    ...
    def test_find_cli_not_found(self):
        ...
        from ripperdoc_agent_sdk._errors import CLINotFoundError
        ...
        with (
            ...
            SubprocessCLITransport(prompt="test", options=RipperdocAgentOptions())

        assert "Ripperdoc Code not found" in str(exc_info.value)
    ...
    def test_build_command_basic(self):
        ...
        assert cmd[0] == "/usr/bin/ripperdoc"
    ...
    def test_cli_path_accepts_pathlib_path(self):
        ...
        path = Path("/usr/bin/ripperdoc")
        transport = SubprocessCLITransport(
            prompt="Hello",
            options=RipperdocAgentOptions(cli_path=path),
        )
    ...
    def test_build_command_with_options(self):
        ...
        assert "Bash" in cmd
        assert "--model" in cmd
        assert "ripperdoc-sonnet-4-5" in cmd
        ...
        async def _test():
            with patch("anyio.open_process") as mock_exec:
                ...
                mock_version_process.stdout.receive = AsyncMock(
                    return_value=b"2.0.0 (Ripperdoc Code)"
                )
    ...
    def test_env_vars_passed_to_subprocess(self):
        ...
        async def _test():
            ...
            with patch(
                ...
                assert "RIPPERDOC_CODE_ENTRYPOINT" in env_passed
                assert env_passed["RIPPERDOC_CODE_ENTRYPOINT"] == "sdk-py"
    ...
    def test_build_command_with_sandbox_only(self):
        ...
        from ripperdoc_agent_sdk import SandboxSettings
    ...
    def test_build_command_with_tools_preset(self):
        ...
        transport = SubprocessCLITransport(
            prompt="test",
            options=make_options(tools={"type": "preset", "preset": "ripperdoc_code"}),
        )
    ...
    def test_concurrent_writes_are_serialized(self):
        ...
        async def _test():
            ...
            try:
                transport = SubprocessCLITransport(
                    prompt="test",
                    options=RipperdocAgentOptions(cli_path="/usr/bin/ripperdoc"),
                )
    ...
    def test_build_command_agents_always_via_initialize(self):
        ...
        """
        from ripperdoc_agent_sdk.types import AgentDefinition
